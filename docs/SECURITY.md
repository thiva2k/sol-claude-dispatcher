...
| **Core deny set is non-configurable** | **HARD** *(in one specific sense — read the next row)* | `runner.ALWAYS_DISALLOWED_TOOLS` = `("mcp__*", "Agent", "Task", "Bash(claude:*)", "Bash(codex:*)", "Bash(gh:*)")` **+** `CORE_DENIED_GIT_OPERATIONS` = `Bash(git push:*)`, `Bash(git merge:*)`, `Bash(git rebase:*)`, `Bash(git commit:*)`, `Bash(git reset:*)`, `Bash(git clean:*)`, `Bash(git worktree:*)`, `Bash(git bisect:*)` (eight entries — `git bisect run <cmd>` executes an arbitrary command at every bisection step and detaches HEAD, and was reached by none of the first seven prefixes; `Bash(gh:*)` closes the one common tool that reaches past the local tree to mutate remote GitHub state with the operator's stored credentials). Both builders append them unconditionally via `_deduped(config.claude.disallowed_tools, ALWAYS_DISALLOWED_TOOLS)`, and `_assert_invocation_sane()` refuses to build argv for an invocation missing any of them. **Operator configuration may ADD deny rules; it can never REMOVE one of these.** Editing `dispatcher.toml` — even to `disallowed_tools = []` — changes nothing about this set. |
...
| **`--safe-mode` disables the operator's customization surface** | **HARD** *(that the flag is emitted and enforced by the CLI at process start — not an OS boundary)* | `runner.build_argv()` emits `--safe-mode` unconditionally for both roles (worker and Fable) and on resume, gated only by `CLI_CAPABILITIES["safe_mode"]` — an operator cannot suppress it via `dispatcher.toml`, there is no config key for it. The Claude CLI enforces it at its own process start (the child sets `CLAUDE_CODE_SAFE_MODE=1`); the dispatcher does not re-check it afterwards. The child therefore starts with `CLAUDE.md`, skills, plugins, hooks, MCP servers, custom commands/agents, output styles, workflows, custom themes and keybindings all disabled, while auth, model selection, built-in tools and permissions keep working — so everything else in this table still applies unchanged. It is invocation-specific and **not** an OS boundary: it suppresses Claude Code's own customization loading, nothing else, and the user's interactive `claude` session is untouched. |
