...
def claim_I_size(gate) -> None:
    """Grow the fixture to the measured production worst case and run it live.

    This is the claim that found a blocking defect. Read `I-1` and `I-2` before
    trusting any size assumption elsewhere in Gate 4.5.
    """
    worst = production_worst_case()
    limit = max_single_argv_bytes()
    eprint(f"[claim I] production worst case: {worst}")
    eprint(f"[claim I] measured single-argv limit: {limit}")

    import manifests as manifests_mod

    def rebuild(skill_pad: int, guidance_pad: int) -> tuple[list[str], Any, int]:
        manifests_mod.build_skill_manifest(
            repo=gate.fx.repo,
            out_path=gate.fx.skills_manifest_path,
            pad_core_to=skill_pad,
        )
        manifests_mod.build_guidance_manifest(
            repo=gate.fx.repo,
            out_path=gate.fx.guidance_manifest_path,
            artifact_dir=gate.artifact_dir,
            tokens=gate.proj_tokens,
            known_foreign=("SubAgent/deep/nested/CLAUDE.md",),
            pad_root_to=guidance_pad,
        )
        gate.config = load_config(gate.fx.config_path)
        argv, spec = gate.worker_argv(
            "Using only the context you already have, state the curated root "
            "reference value, on one line, and nothing else.",
            gate.fx.repo,
        )
        return argv, spec, len((spec.append_system_prompt or "").encode())

    # -- I-0: the limit itself --------------------------------------------
    ev0 = write_evidence(
        gate.fx.evidence_dir, "claim-I.argv-limit.txt",
        f"measured MAX_ARG_STRLEN for one argv element: {limit} bytes\n"
        f"production worst-case composed context: {worst['worker_worst_total']} bytes\n"
        f"configured ceiling (skills cap + guidance cap + preamble + policy): "
        f"{worst['skills_cap'] + 60000 + worst['preamble'] + worst['worker_policy']} bytes\n"
        + json.dumps(worst, indent=2),
    )
    gate.check(
        "I-0", "argv size limit (§9 I)",
        f"the host caps a SINGLE argv element at {limit:,} bytes, and the "
        f"dispatcher emits --append-system-prompt inline as one element",
        limit > 0, ev0, f"measured by bisection against /bin/true",
    )

    # -- I-1: production worst case, live ---------------------------------
    argv, spec, composed = rebuild(worst["skills_worst"], worst["guidance_worst"])
    inv = run_cli(
        "size/production-worst-case", argv, cwd=gate.fx.repo,
        env=dict(spec.env), timeout_s=420, ledger=gate.ledger,
    )
    started = inv.exit_code is not None
    ev = write_evidence(
        gate.fx.evidence_dir, "claim-I.size.json",
        json.dumps(
            {
                "production_worst_case": worst,
                "measured_single_argv_limit": limit,
                "fixture_composed_append_system_prompt_bytes": composed,
    ...
    gate.check(
        "I-1", "composed prompt size, live (§9 I)",
        f"the real CLI can be LAUNCHED with a composed --append-system-prompt "
        f"sized at the production worst case ({composed:,} bytes)",
        started, ev,
        f"limit={limit:,} composed={composed:,} "
        f"exit={inv.exit_code} stderr={inv.stderr[:200]!r}",
    )
    gate.check(
        "I-2", "configured ceiling vs argv limit (§9 I)",
        f"the CONFIGURED ceiling (skills {worst['skills_cap']:,} + guidance "
        f"60,000 + preamble {worst['preamble']:,} + policy "
        f"{worst['worker_policy']:,}) fits inside the {limit:,}-byte argv limit",
        worst["skills_cap"] + 60_000 + worst["preamble"] + worst["worker_policy"]
        <= limit,
        ev0,
        f"configured ceiling="
        f"{worst['skills_cap'] + 60000 + worst['preamble'] + worst['worker_policy']:,}"
        f" vs limit={limit:,}",
    )

    # -- I-3: the largest size that DOES start ----------------------------
    # Shrink the skill pack until the composed context fits under the limit,
    # keeping the full production-sized guidance. Measured by rebuilding, not
    # by arithmetic: the composer adds delimiters and section headers whose
    # exact cost is its business, not this harness's to predict.
    target = limit - 2048
    guidance_pad = worst["guidance_worst"]
    skill_pad = worst["skills_worst"]
    argv2, spec2, composed2 = rebuild(skill_pad, guidance_pad)
    while composed2 > target and skill_pad > 2048:
        skill_pad -= max(2048, composed2 - target)
        skill_pad = max(2048, skill_pad)
        argv2, spec2, composed2 = rebuild(skill_pad, guidance_pad)
    inv2 = run_cli(
        "size/just-under-limit", argv2, cwd=gate.fx.repo,
        env=dict(spec2.env), timeout_s=420, ledger=gate.ledger,
    )
    ev2 = write_evidence(
        gate.fx.evidence_dir, "claim-I.under-limit.json",
        json.dumps(
            {
                "composed_bytes": composed2,
                "limit": limit,
                "invocation": inv2.to_record(),
                "raw_stdout": inv2.stdout,
                "raw_stderr": inv2.stderr,
            },
            indent=2,
        ),
    )
    accepted = (
        inv2.exit_code == 0
        and inv2.parsed is not None
        and not inv2.parsed.get("is_error", True)
        and inv2.parsed.get("num_turns", 0) > 0
    )
    gate.check(
        "I-3", "composed prompt size, live (§9 I)",
        f"a composed prompt of {composed2:,} bytes — just under the "
        f"{limit:,}-byte limit — IS accepted and produces a real turn",
        accepted, ev2,
        f"exit={inv2.exit_code} num_turns={(inv2.parsed or {}).get('num_turns')}",
    )
    gate.check(
        "I-4", "large prompt fidelity (§9 I)",
        f"at {composed2:,} bytes the curated root value "
        f"{gate.proj_tokens['PROJROOT']} still comes back — a large prompt is "
        f"not silently truncated",
        gate.proj_tokens["PROJROOT"] in (inv2.stdout + inv2.stderr), ev2,
        f"result={inv2.result_text[:200]!r}",
    )

    gate.size_facts = getattr(gate, "size_facts", {})
    gate.size_facts.update(
        {
            "measured_single_argv_limit_bytes": limit,
            "production_worst_composed_bytes": worst["worker_worst_total"],
            "production_guidance_worst_bytes": worst["guidance_worst"],
            "production_skills_worst_profile_bytes": worst["skills_worst"],
            "configured_ceiling_bytes": (
                worst["skills_cap"] + 60_000 + worst["preamble"]
                + worst["worker_policy"]
            ),
            "largest_composed_that_started_bytes": composed2 if accepted else "n/a",
            "size_probe_cost_usd": round(inv.cost_usd + inv2.cost_usd, 6),
        }
    )
